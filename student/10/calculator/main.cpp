...
//         TODO: Implement command execution here!
        ...
        if (command_to_be_executed.length() != 1) {
            command_to_be_executed = capitalize(command_to_be_executed);
        }

        Command new_command = { "" , 2 , false , nullptr };
        double left, right;

        for (auto item : COMMANDS) {
            if (item.str == command_to_be_executed) {
                new_command = item;
                break;
            }
        }
        ...
        if ( new_command.action == nullptr && pieces.size() != 1 ) {
            cout << "Error: wrong number of parameters." << endl;
            continue;
        }

        cout << new_command.action(left, right) << endl;
    }
}
